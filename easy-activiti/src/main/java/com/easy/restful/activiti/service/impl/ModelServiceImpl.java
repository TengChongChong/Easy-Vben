...
import com.easy.restful.activiti.dao.ModelMapper;
...
public class ModelServiceImpl extends ServiceImpl<ModelMapper, Model> implements ModelService {
    ...
    @Override
    public Page<Model> select(Model model, Page<Model> page) {
    ...
    public ModelEntity saveData(Model model) {
        ...
        if (checkKeyIsExistence(model.getKey(), model.getId())) {
        ...
        setModelEditorSource(model);

        return modelEntity;
    }

    private void setModelEditorSource(Model model) {
        if (StrUtil.isNotBlank(model.getPath())) {
            InputStream bpmnStream;
            try {
                bpmnStream = new FileInputStream(model.getPath());
            ...
            properties.put(ModelConst.PROPERTIES_DOCUMENTATION, model.getDescription());
            ...
            editorNode.set(ModelConst.PROPERTIES, properties);
    ...
    public String export(String id) {
        ...
        // 写入文件
        String bpmnPath = com.easy.restful.util.file.FileUtil.getTemporaryPath() + IdUtil.randomUUID() + ".bpmn20.xml";
